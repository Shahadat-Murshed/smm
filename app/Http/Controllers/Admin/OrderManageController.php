...
class OrderManageController extends Controller
{
    ...
    {
        $user = User::with('order', 'order.category',)->findOrFail($id);
        $userid = $user->id;
    ...
    public function userOrderSearch(Request $request)
    {
        ...
        $orders = AdvOrder::when($userid, function ($query) use ($userid) {
            return $query->where('user_id', $userid);
        })
            ->when($search, function ($query) use ($search) {
                return $query->where('id', 'LIKE', "%{$search}%")
                    ->orWhereHas('category', function ($q) use ($search) {
                        return $q->where('category_title', 'LIKE', "%{$search}%");
                    })
    ...
    public function usersOrderChangeStatus(Request $request)
    {
        ...
        if ($request->strIds == null) {
            ...
            if (count($ids) > 0) {
                $logs = AdvOrder::whereIn('id', $ids)->with('user')->get()->map(function ($order) use($status){
                    $order->status = $status;
                    $order->save();
