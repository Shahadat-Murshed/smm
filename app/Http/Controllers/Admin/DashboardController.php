...
class DashboardController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $data['totalOrder'] = AdvOrder::count();
        ...
        $orders = AdvOrder::where('created_at', '>', Carbon::now()->subDays(30))
        ...
        $data['bestSale'] = AdvOrder::with('category')
            ->whereHas('category')
            ->selectRaw('category_id ,COUNT(category_id) as count, sum(total_quantity) as quantity',)
            ->groupBy('category_id')->orderBy('count', 'DESC')->take(10)->get();
        
        
        $data['bestProvider'] = ApiProvider::findorFail(1);

        // dd($data['bestProvider']);

        $orderStatistics = AdvOrder::where('created_at', '>', Carbon::now()->subDays(30))
            ->selectRaw('count(CASE WHEN status = "completed"  THEN status END) AS completed')
            ->selectRaw('count(CASE WHEN status = "processing"  THEN status END) AS processing')
            ->selectRaw('count(CASE WHEN status = "pending"  THEN status END) AS pending')
    ...
    public function referralCommissionStore(Request $request)
    {
        ...
        Referral::where('commission_type',$request->commission_type)->where('user_id', $request->user_id)->delete();
    ...
    {
        $data['tiers'] = Tier::all()->sortBy('level');
        return view('admin.pages.tiers.index', $data);
    }
