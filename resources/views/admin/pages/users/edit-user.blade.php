            ...
            <div class="col-sm-8">
                ...
                <div class="card ">
                    <div class="card-body">
                        ...
                        <h4 class="card-title text-white">@lang('Promo Code')
                            <button type="button"
                                    class="btn btn-primary btn-sm  text-white float-right  waves-effect generatePromoBtn"><i
                                    class="fa fa-spinner"></i> @lang('Generate New Code')
                            </button>
                        </h4>

                        <form action="{{route('admin.promo-update', $user->id)}}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="form-group ">
                                <h6 class="font-weight-bold">@lang('Promo Code')</h6>
                                <div class="input-group">  
                                    <input name="promo_code" type="text" value="{{$user->promo_code}}"
                                        class="form-control form-control-lg api-token" id="promo_code">
                                    <div class="input-group-append">
                                        <span class="input-group-text copytext copyPromo" id="copyPromo">
                                                <i class="fa fa-copy"></i>
                                        </span>
                                    </div>
                                    <button type="submit" class="ml-5 btn waves-effect waves-light btn-rounded btn-primary">
                                        <span>@lang('Update Promo Code')</span></button>
                                </div>
                            </div>
                        </form>
                    </div>
...
@push('js')
    <script>
        ...
        $('.generatePromoBtn').on('click', function () {
            $.ajax({
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf_token"]').attr('content')},
                url: "{{route('admin.promo-generate',[$user->id])}}",
                type: 'POST',
                success(data) {
                    $("#promo_code").val(data)
                    Notiflix.Notify.Success("KEY GENERATE: " + data);
