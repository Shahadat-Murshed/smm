...
@section('content')
        ...
        <div class="card-body">
            ...
            <div class="dropdown mb-2 text-right">
                ...
                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                    ...
                    <button class="dropdown-item" type="button" data-toggle="modal"
                            data-target="#promo_inactive">@lang('Promo Deactive')</button>
                    <button class="dropdown-item" type="button" data-toggle="modal"
                            data-target="#promo_active">@lang('Promo Active')</button>
                </div>
            </div>
            <div class="table-responsive">
                <table class="categories-show-table table table-hover table-striped table-bordered">
                    ...
                    <tr>
                        ...
                        <th scope="col">@lang('Tier Earnings')</th>
                        <th scope="col">@lang('Promo Active?')</th>
                    ...
                    @forelse($users as $user)
                        <tr>
                            ...
                            <td class="align-middle text-center" data-label="@lang('No.')">{{loopIndex($users) + $loop->index	 }}</td>
                            <td class="align-middle text-center" data-label="@lang('Username')">@lang($user->username)</td>
                            <td class="align-middle text-center" data-label="@lang('Email')">@lang($user->email)</td>
                            <td class="align-middle text-center" data-label="@lang('Phone')">@lang(($user->phone)? : 'N/A')</td>
                            <td class="align-middle text-center" data-label="@lang('Balance')">{{getAmount($user->balance, config('basic.fraction_number'))}} {{trans(config('basic.currency'))}}</td>
                            <td class="align-middle text-center" data-label="@lang('Username')">@lang($user->role)</td>
                            <td class="align-middle text-center" data-label="@lang('Percentages %')">
                                {{$user->base_percentage ?: 'N/A'}}
                            </td>
                            <td class="align-middle text-center" data-label="@lang('Tier_earnings')">
                                @php $tier = $user?->tier()?->latest()?->first(); @endphp
                                {{ $tier?->tier?->name ?: 'N/A' }} - {{ $tier?->current_amount ?: 0 }}
                            </td>
                            <td class="align-middle text-center" data-label="@lang('Promo_status')">
                                <span
                                    class="badge badge-pill {{ $user->is_promo_active == 0 ? 'badge-danger' : 'badge-success' }}">{{ $user->is_promo_active == 0 ? 'No' : 'Yes' }}</span>
                            </td>                            
                            <td class="align-middle text-center" data-label="@lang('Status')">
                                <span
                                    class="badge badge-pill {{ $user->status == 0 ? 'badge-danger' : 'badge-success' }}">{{ $user->status == 0 ? 'Inactive' : 'Active' }}</span>
                            </td>                            
                            <td class="align-middle text-center" data-label="@lang('Action')">
                                <div class="dropdown show dropup">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                        ...
                                        <a class="dropdown-item" href="{{ route('admin.user-role',$user->id) }}">
                                            <i class="fas fa-pen-fancy text-warning pr-2"
                                               aria-hidden="true"></i>@lang('Change Role')
                                        </a>
    ...
    <!-- Active/Inactive Modals End -->

    <!-- Promo Active/Deactive Modals Starts -->
    <div class="modal fade" id="promo_active" role="dialog">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header modal-colored-header bg-primary">
                    <h5 class="modal-title">@lang('Active User Promo')</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        ×
                    </button>
                </div>
                ...
                <div class="modal-body">
                    ...
                    <p>@lang("Are you sure to grant the User Promo Privilage")</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-light" data-dismiss="modal"><span>@lang('No')</span></button>
                    <form action="" method="post">
                        @csrf
                        <a href="" class="btn btn-primary promo-active-yes"><span>@lang('Yes')</span></a>
                    </form>
                </div>
            </div>
        </div>
    </div>
    <div class="modal fade" id="promo_inactive" role="dialog">
        <div class="modal-dialog">

            <div class="modal-content">
                <div class="modal-header modal-colored-header bg-primary">
                    <h5 class="modal-title">@lang('DeActive User Promo')</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        ×
                    </button>
                </div>
                ...
                <div class="modal-body">
                    <p>@lang("Are you sure to deny the User Promo Privilage")</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-light" data-dismiss="modal"><span>@lang('No')</span></button>
                    <form action="" method="post">
                        @csrf
                        <a href="" class="btn btn-primary promo-inactive-yes"><span>@lang('Yes')</span></a>
                    </form>
                </div>
            </div>
        </div>
    </div>
    <!-- Promo Active/Deactive Modals End -->
...
@push('js')
    <script>
        ...
        $(document).ready(function () {
            ...
            //multiple promo active
            $(document).on('click', '.promo-active-yes', function (e) {
                e.preventDefault();
                var allVals = [];
                $(".row-tic:checked").each(function () {
                    allVals.push($(this).attr('data-id'));
                ...
                $.ajax({
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf_token"]').attr('content')},
                    url: "{{ route('admin.promo-multiple-active') }}",
                    data: {strIds: strIds},
                    dataType: 'json',
                    type: "post",
                    success: function (data) {
            ...
            //multiple promo deactive
            $(document).on('click', '.promo-inactive-yes', function (e) {
                e.preventDefault();
                var allVals = [];
                $(".row-tic:checked").each(function () {
                    allVals.push($(this).attr('data-id'));
                ...
                $.ajax({
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf_token"]').attr('content')},
                    url: "{{ route('admin.promo-multiple-inactive') }}",
                    data: {strIds: strIds},
                    dataType: 'json',
                    type: "post",
                    success: function (data) {
