...
Route::view('/packages','v2/packages')->name('packages');
Route::view('/services','v2/services')->name('services');
Route::view('/terms','v2/terms')->name('terms');
Route::view('/privacy','v2/privacy')->name('privacy');
Route::view('/refund','v2/refund')->name('refund');
Route::view('/buy','buy')->name('buy');

Route::post('/payment/callback', function(){
    dd('$asd');
});
Route::post('order/package','CartController@processPackage')->name('order.package');
Route::post('order/service','CartController@processPackage')->name('order.service');
//Route::get('cart','CartController@create');

Route::get("/drip/test", function (DripOrderService $dripOrderService) {
...
Route::group(['prefix' => 'admin', 'as' => 'admin.'], function () {
    ...
    Route::group(['middleware' => ['auth:admin']], function () {
        ...
        Route::get('/tiers','Admin\DashboardController@tiers')->name('tiers');
        ...
        Route::group(['middleware' => ['isSuperAdmin'], 'prefix' => 'super', 'as' => 'super.'], function () {
        ...
        Route::post('/promos-active', 'Admin\UsersController@promoActiveMultiple')->name('promo-multiple-active');
        Route::post('/promos-inactive', 'Admin\UsersController@promoInactiveMultiple')->name('promo-multiple-inactive');
        Route::post('/promo-generate/{id}', 'Admin\UsersController@promoGenerate')->name('promo-generate');
        Route::put('/promo-update/{id}', 'Admin\UsersController@promoUpdate')->name('promo-update');
        ...
        Route::get('/user/role-change/{id}', 'Admin\UsersController@userRoleEdit')->name('user-role');
        Route::put('/user/role-update/{id}', 'Admin\UsersController@userRoleUpdate')->name('user-role.update');
        ...
        Route::get('/advanced-services/append', 'AdvancedServiceController@appendNewProviderView')->name('advanced-services.append');
        Route::resource('/advanced-services', 'AdvancedServiceController');

        Route::get('/package/append-service', 'PackageController@appendService')->name('package.append-service');
        Route::get('/search-advService', 'PackageController@searchAdvServices')->name('package.search-advService');
...
Route::middleware('Maintenance')->group(function () {
    ...
    /** Affiliate Routes Through Link **/
    Route::group(['middleware' => ['guest']], function () {
        Route::get('register/{sponsor?}/{hash?}', 'Auth\RegisterController@sponsor')->name('register.sponsor');
    });

    /** Affiliate Routes Direct Register **/
    Route::group(['middleware' => ['guest']], function () {
        Route::get('register-as-affiliate/', 'Auth\RegisterController@affiliate')->name('register-as-affiliate');
    });
    ...
    Route::group(['middleware' => ['auth'], 'prefix' => 'user', 'as' => 'user.'], function () {
        ...
        Route::middleware('userCheck')->group(function () {
            ...
            Route::get('order/v2/getProviderView', 'User\AdvancedOrderController@getProviderView')->name('order.provider.view');
            Route::resource('orderV2', 'User\AdvancedOrderController');
            ...
            //Affiliate Zone Routes
            Route::get('refer/affiliate','User\ReferralController@create')->name('refer.affiliate.create');
            Route::post('refer/affiliate', 'User\ReferralController@store')->name('refer.affiliate.store');
            Route::get('/packages', 'User\AffiliateZoneController@packages')->name('affiliate.package');
            ...
            Route::get('adv/service', function (Request $request) {
                $parent_id = $request->cat_id;
                $services = AdvancedService::where('category_id', $parent_id)->where('service_status', 1)->get();
                return response()->json($services);
            })->name('adv.service');
            ...
            Route::get('advanced/user-service', 'User\AdvancedOrderController@userservice')->name('adv.service_id');
            Route::get('/advanced/providers', 'User\AdvancedOrderController@getProviderDetails')->name('adv.get_providers');
    ...
    Route::get('/service', 'FrontendController@services')->name('service');
